...
import { AlertTriangle, CreditCard, Network, Trash2 } from "lucide-react";
...
function WorkspaceBillingBanner() {
  ...
  const activeReleaseRole = useFlowStore((state) => state.updater.releaseChannel);
      ...
      <div className="pointer-events-auto space-y-3 rounded-[28px] border border-[#6a4b08] bg-[#17120a]/95 p-4 shadow-[0_28px_100px_rgba(0,0,0,0.35)] backdrop-blur-md">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="text-[10px] uppercase tracking-[0.16em] text-[#d29922]">Pagamento pendente</div>
            ...
            <div className="mt-2 text-[13px] leading-6 text-[#c9d1d9]">
              O acesso sera bloqueado em {formatDateLabel(license.timeline.paymentDueAt)} se o PIX nao for confirmado.
            </div>
          </div>
        </div>
            ...
            {canUseInternalBillingActions ? (
              <>
                ...
                >
                  ...
                  <div className="mt-1 text-xs text-[#7ee787]">Bypass de pagamento para internal</div>
                </button>
                <button
                  type="button"
                  disabled={accountPending}
                ...
                >
                  ...
                  <div className="mt-1 text-xs text-[#7ee787]">Ativa o lifetime sem PIX real</div>
                </button>
              </>
            ) : null}
          </div>
      ...
      footer={
        <div className="flex items-center justify-between gap-3">
          <div className="text-[11px] text-[#7d8590]">
            O acesso volta assim que um pagamento for confirmado antes do prazo final.
          </div>
          <button
            type="button"
            onClick={() => {
    ...
    >
      <div
        className={`grid gap-6 ${
          lockHasSidePanel ? "lg:grid-cols-[minmax(0,1fr)_320px]" : "grid-cols-1"
        }`}
      >
          <div>
            ...
            <div className="mt-4 max-w-[58ch] text-[15px] leading-7 text-[#8b949e]">
              Seus dados continuam locais nesta instalacao, mas o workspace so volta a abrir depois do PIX confirmar.
              Se nada for pago ate {formatDateLabel(license.timeline.deleteAt)}, a conta e os dados locais serao removidos.
            </div>

            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <div className="rounded-2xl border border-[#30363d] bg-[#0d1117] p-4">
                <div className="text-[10px] uppercase tracking-[0.16em] text-[#7d8590]">Bloqueado em</div>
                <div className="mt-2 text-sm font-medium text-[#f0f6fc]">
                  {formatDateLabel(license.timeline.blockedAt)}
                </div>
              </div>
              <div className="rounded-2xl border border-[#30363d] bg-[#0d1117] p-4">
                <div className="text-[10px] uppercase tracking-[0.16em] text-[#7d8590]">Delete final</div>
                <div className="mt-2 text-sm font-medium text-[#f0f6fc]">
            ...
            {!license.billing.activeCharge ? (
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                ...
                >
                  <div className="text-sm font-semibold text-[#f0f6fc]">Pro Mensal</div>
                  <div className="mt-2 text-[13px] leading-6 text-[#8b949e]">R$89 por ciclo, cobranca manual via PIX.</div>
                </button>
                <button
                  type="button"
                  disabled={billingPending}
                ...
                >
                  <div className="text-sm font-semibold text-[#f0f6fc]">Founder Lifetime</div>
                ...
                {canUseInternalBillingActions ? (
                  <>
                    ...
                    >
                      <div className="text-sm font-semibold text-[#d2f8dd]">Simular mensal</div>
                      <div className="mt-2 text-[13px] leading-6 text-[#7ee787]">Confirma o pagamento sem PIX real.</div>
                    </button>
                    <button
                      type="button"
                      disabled={accountPending}
                      ...
                      className="rounded-[24px] border border-[#2f6f3e] bg-[#102019] p-4 text-left transition-colors hover:bg-[#143224] disabled:opacity-60"
                    >
                      <div className="text-sm font-semibold text-[#d2f8dd]">Simular vitalicio</div>
                      <div className="mt-2 text-[13px] leading-6 text-[#7ee787]">Ativa o lifetime para teste interno.</div>
                    </button>
                  </>
                ) : null}
              </div>
          ...
          {license.billing.activeCharge ? (
            <div className="space-y-3">
              ...
              {canUseInternalRole ? (
                ...
                >
                  <div className="font-medium">Simular pagamento deste PIX</div>
                  <div className="mt-1 text-xs text-[#7ee787]">Atalho de teste para contas internal.</div>
                </button>
              ) : null}
            </div>
          ) : null}
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#0d1117] px-4 py-10">
      <div className="w-full max-w-[520px] rounded-[32px] border border-[#30363d] bg-[#11161d] p-6 text-center shadow-[0_28px_110px_rgba(0,0,0,0.36)]">
        ...
        <div className="mt-4 text-[10px] uppercase tracking-[0.18em] text-[#7d8590]">Conta removida</div>
        ...
        <div className="mt-4 text-[15px] leading-7 text-[#8b949e]">
          Para voltar a usar o Flow Merge, faca login de novo e gere uma nova cobranca.
        </div>
      </div>
    </div>
  );
...
export function FlowMergeShell() {
  ...
  const showDeletedState = license.shouldWipeLocalData || license.accessState === "deleted";

  return (
    <>
      <DesktopUpdateBridge />
      {!license.authenticated ? <LandingPage /> : null}
      {license.authenticated && showDeletedState ? <DeletedAccountScreen /> : null}
      {license.authenticated && !showDeletedState && license.canAccessWorkspace ? (
        <>
          <CanvasEntry />
          <WorkspaceBillingBanner />
        </>
      ) : null}
      {license.authenticated && !showDeletedState && !license.canAccessWorkspace ? <BillingLockScreen /> : null}
    </>
  );
}
